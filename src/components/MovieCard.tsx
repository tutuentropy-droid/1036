...
import { Calendar, MapPin, User, Tag, Shuffle, Quote, MessageSquare } from 'lucide-react';
...
export default function MovieCard({ movie, index }: MovieCardProps) {
  ...
  const { getReviewCountByMovie } = useReviewStore();
  const prevRewindKey = useRef(rewindKey);

  const misalignment = computeMovieMisalignment(movie);
  const reviewCount = getReviewCountByMovie(movie.id);
  ...
  return (
    <div 
      className={`opacity-0 animate-fade-in-up ${delayClass} relative group`}
    >
      <div 
        ...
        >
          ...
          >
            ...
            >
              ...
              {reviewCount > 0 && (
                <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 bg-amber-100/90 rounded text-xs font-terminal text-amber-800">
