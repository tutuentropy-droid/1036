...
export function generateScript(movieA: Movie, movieB: Movie): Script {
  ...
  const shuffledOpening = shuffle(openingPool);
  const shuffledResponse = shuffle(responsePool);
  const shuffledMidTalk = shuffle(midTalkPool);
  const shuffledClosing = shuffle(closingPool);
  ...
  lines.push({
    ...
    text: injectMovieContext(shuffledOpening[0], movieA, movieB),
  ...
  lines.push({
    ...
    text: injectMovieContext(shuffledResponse[0], movieB, movieA),
  ...
  for (let i = 0; i < lineCount - 4; i++) {
    ...
    const text = shuffledMidTalk[midIndex % shuffledMidTalk.length];
    midIndex++;
  ...
  lines.push({
    ...
    movieTitle: movieA.title,
    movieEmoji: movieA.emoji,
    text: injectMovieContext(shuffledClosing[0], movieA, movieB),
    timestamp: (timestamp += 2000),
  });

  const closingB = shuffledClosing.length > 1 ? shuffledClosing[1] : shuffledClosing[0];
  lines.push({
    id: generateLineId(),
    speaker: 'B',
    movieTitle: movieB.title,
    movieEmoji: movieB.emoji,
    text: injectMovieContext(closingB, movieB, movieA),
